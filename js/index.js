...
function addAlumno( pnombre, pdni, pcurso, ptelefono, pusuario, pcontraseña){
...
function getAlumno(){
    ...
    var storedAlumnos = localStorage.getItem('localAlumnos')

    if (storedAlumnos == null){
        Alumnos = [];
    }else{
...
function localStorageAlumnos(plist){
    localStorage.setItem('localAlumnos', JSON.stringify(plist))
}


function encontrarAlumno(pdni){
    var alumnoObj
    for(var i = 0; i < Alumnos.length;i++){
        if(Alumnos[i].dni == pdni){
            alumnoObj = Alumnos[i];
        }
    }
    return alumnoObj;
...
function addMateriaToAlumnos(palumnoObj, pMateriaName, pMateriaProfesor, pMateriaDivision, pMateriaNota){
    var newMateria = {
        name : pMateriaName,
        profesor : pMateriaProfesor,
        division : pMateriaDivision,
        nota : pMateriaNota

    }
    var index = Alumnos.indexOf(palumnoObj);
    palumnoObj.materiasList.push(newMateria);

    materiasList[index] = palumnoObj;
    localStorageAlumnos(Alumnos);
